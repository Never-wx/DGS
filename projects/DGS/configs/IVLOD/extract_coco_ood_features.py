"""
extract_coco_ood_features.py

Extract GDINO neck features from COCO val2017 as OOD reference samples.
...
def parse_args():
    parser = argparse.ArgumentParser(description='Extract GDINO features from COCO val2017 for OOD evaluation')
    ...
    parser.add_argument('--save-dir', default='visualize/CDIOD_mlvlfeat/COCO_val')
...
def main():
    ...
    if len(existing) >= args.max_samples:
        print(f"Already extracted {len(existing)} COCO OOD features. Skipping.")
        return

    # Build model using group_moe_extract_feats config (all frozen, no domain predictor)
    cfg = Config.fromfile('projects/DGS/configs/IVLOD/extract_feats.py')
    ...
    # Build COCO val2017 dataset using ZCOCO config structure
    coco_ds_cfg = dict(
        type='CocoDataset',
    ...
    print("Building COCO val2017 dataset...")
    dataset = DATASETS.build(coco_ds_cfg)
    print(f"Total COCO val images: {len(dataset)}")
    ...
    indices.sort()
    print(f"Extracting {len(indices)} COCO OOD features to {args.save_dir}...")
    ...
    print(f"Done! {final_cnt} COCO OOD features saved to {args.save_dir}")
