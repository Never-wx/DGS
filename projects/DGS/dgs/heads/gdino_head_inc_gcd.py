...
from ..losses import inter_query_relation, inter_text_relation_partial
...
class GroundingDINOHead_inc_gcd(GroundingDINOHead_inc):
    ...
    def __init__(self, distn_cfg, **kwargs):
        ...
        if self.distn_cfg.label_distn.type == 'topk_pseudo' or self.distn_cfg.label_distn.type == 'threshold_pseudo':             
            self.sigma = self.distn_cfg.label_distn.get('sigma', 0.4)
            self.label_iou_th = self.distn_cfg.label_distn.get('label_iou_th', 0.7)
    ...
    def loss_by_feat_old(
        ...
        # ===== logits distn loss =====    
        if self.distn_cfg.label_distn.loss_ld.type != 'None':
